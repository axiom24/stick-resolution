...
import MakeYourResolutionStickArtifact from './MakeYourResolutionStick.json';
...
class Main extends Component {
  ...
  initContractInstance() {
    ...
    if (typeof web3 !== 'undefined') {
      ...
      this.contractInstance = contract(MakeYourResolutionStickArtifact);
  ...
  render() {
    ...
    if (this.state.step === 1) {
      title = <h1>what's your new year resolution?</h1>;

      formInner = (<div>
        <input type="text" name="description" ref={input => this.descriptionInput = input} onFocus={this.focusIn} onBlur={this.focusOut} placeholder="Quit Smoking" value={this.state.description} onChange={this.handleChange} required />
        <button onClick={this.goNext}><i className="fa fa-bolt" aria-hidden="true"></i> Set Your Resolution</button>
      </div>);
    } else if (this.state.step === 2) {
      title = <h1>When</h1>;
      ...
      formInner = (<div>
        <div id="confirm-commitment">
          ...
          <p>If they state you have reached your goal, "Make Your Resolution Stick" smart contract deployed on Ethereum mainnet will automatically transfer your funds back automatically. If they don't, it will take the balance of {this.state.amount} ETH.</p>
        </div>

        <button onClick={this.goNext}><i className="fa fa-check" aria-hidden="true"></i> Agree To Make It Stick</button>
      </div>);  
    } else if (this.state.step === 7) {
      title = <h1>Done  ! Now let's make it stick</h1>
